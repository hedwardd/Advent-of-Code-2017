lines = """1640	590	93	958	73	1263	1405	1363	737	712	1501	390	68	1554	959	79
...
while i < len(lines):
	...
	## for every column that may be dividend
	while (column <= len(thisRow)-1) & (found == False):
		
		## set possible dividend to this column
		dividend = thisRow[column]


		## for every column that may be divisor
		while(found == False):
			print(1)

			## possible divisor to this column2
			dividend = thisRow[column]

			## check dividend and divisor
			if (dividend != thisRow[column]) & (int(dividend) % int(thisRow[column]) == 0):
				found = True
				divisor = thisRow[column]
	...
	## add up checksum
	checksum = checksum + int(dividend) / int(divisor)
